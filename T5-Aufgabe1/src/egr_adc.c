...
void ADC_Init(int length, int mode)
{
    ...
    // choose ADC clock source (internal clock)  P328
    // 2.选择ADC的时钟源（内部时钟）   这个要在STM32G4 Series advanced Arm®-based 32-bit MCUs - Reference manual的文件中才能找到  P328
    select_adc_clock();
    
    // activate GPIO module and configure pins as analog mode
    // 3.激活GPIO模块并配置引脚为模拟模式
    activate_sensor_gpio();

    // 4.配置ADC
    ADC1_configuration();
    ADC2_configuration(length, mode);
...
// ADC1 configuration
void ADC1_configuration(void)     // P687
{
...
// ADC configuration
void ADC2_configuration(int length, int mode)     // P687
{
    // deep power down mode deactivation
...
// Read Potentiometer value in single mode, and return the result
...
// Read Brightness sensor value in single mode, and return the results
