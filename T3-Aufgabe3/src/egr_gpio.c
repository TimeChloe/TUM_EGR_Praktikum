...
{
    // activate GPIO B module
    uint32_t volatile *adresse = (uint32_t *)(0x40021000 + 0x4C); //0x40021000 is the base address of the RCC module, 0x4C is the offset for the AHB2ENR register
    *adresse |= (1 << 1);
}
void deactivate_gpio_a(void)
{
    // deactivate GPIO A module
    uint32_t volatile *adresse = (uint32_t *)(0x40021000 + 0x4C); //0x40021000 is the base address of the RCC module, 0x4C is the offset for the AHB2ENR register
    *adresse &= ~(1 << 0);
}
void deactivate_gpio_b(void)
{
    // deactivate GPIO B module
    uint32_t volatile *adresse = (uint32_t *)(0x40021000 + 0x4C); //0x40021000 is the base address of the RCC module, 0x4C is the offset for the AHB2ENR register
    *adresse &= ~(1 << 1);
}
...
void gpio_set_pin(uint32_t gpio_base, uint32_t pin) {
    // Set Pin as output
    gpio_mode(gpio_base, pin, GPIO_OUTPUT);

    // 设置GPIO引脚为高电平
    gpio_write(gpio_base, pin, PIN_HIGH);
}
...
void gpio_clear_pin(uint32_t gpio_base, uint32_t pin) {
    ...
    // 设置GPIO引脚为低电平
    gpio_write(gpio_base, pin, PIN_LOW);
}
...
void gpio_alternate_function(uint32_t gpio_base, uint32_t pin, uint32_t af)
{
    ...
    if (pin < 8) {
        // 0x20 is the offset for the GPIO_AFRL (GPIO Alternative Function Low Register)
        adresse = (uint32_t *)(gpio_base + 0x20);
    } else {
        // 0x24 is the offset for the GPIO_AFRH (GPIO Alternative Function High Register)
        adresse = (uint32_t *)(gpio_base + 0x24);
        pin -= 8;  // adjust the pin number for the high register
    }
